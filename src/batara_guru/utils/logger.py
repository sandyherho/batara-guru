"""Simulation logger for Rule 30 simulations."""
...
class SimulationLogger:
    """Logger for Rule 30 simulations."""
    ...
    def _setup_logger(self):
        ...
        logger = logging.getLogger(f"rule30_{self.scenario_name}")
    ...
    def log_results(self, results: dict):
        ...
        self.info(f"  Mean entropy: {results['mean_entropy']:.4f}")
        self.info(f"  Mean complexity: {results['mean_complexity']:.4f}")
        self.info(f"  Final density: {results['final_density']:.4f}")
    ...
    def log_timing(self, timing: dict):
        ...
        self.info("=" * 60)
        self.info("TIMING SUMMARY")
        self.info("=" * 60)
        
        for key, value in sorted(timing.items()):
            display_name = key.replace('_', ' ').title()
            self.info(f"  {display_name}: {value:.2f} s")
